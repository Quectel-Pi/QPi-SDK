...
static int __s5k3l6_stop_stream(struct s5k3l6 *s5k3l6)
{
	return s5k3l6_write_reg(s5k3l6->client,
				 s5k3l6_REG_CTRL_MODE,
				 s5k3l6_REG_VALUE_08BIT,
...
static int __s5k3l6_power_on(struct s5k3l6 *s5k3l6)
{
	...
	ret = regulator_enable(s5k3l6->iovdd_regulator);
	if (ret < 0) {
		dev_err(dev, "Failed to enable IOVDD regulator\n");
		goto disable_iovdd;
	}
	
	ret = regulator_enable(s5k3l6->avdd_regulator);
	if (ret < 0) {
		dev_err(dev, "Failed to enable AVDD regulator\n");
		goto disable_avdd;
	}
   
    ret = regulator_enable(s5k3l6->dvdd_regulator);
	if (ret < 0) {
...
disable_dvdd:
	regulator_disable(s5k3l6->dvdd_regulator);
disable_iovdd:
	regulator_disable(s5k3l6->iovdd_regulator);
disable_avdd:
	regulator_disable(s5k3l6->avdd_regulator);
disable_clk:
...
static int __maybe_unused s5k3l6_runtime_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct s5k3l6 *s5k3l6 = to_s5k3l6(sd);

	return __s5k3l6_power_on(s5k3l6);
}

static int __maybe_unused s5k3l6_runtime_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct s5k3l6 *s5k3l6 = to_s5k3l6(sd);

	__s5k3l6_power_off(s5k3l6);
				...
				struct v4l2_mbus_config *config)
{
	...
	config->bus.mipi_csi2.num_data_lanes = s5k3l6_LANES;
			 ...
			 const struct i2c_device_id *id)
{
	...
	s5k3l6->avdd_regulator = devm_regulator_get(dev, "avdd");
	if (IS_ERR(s5k3l6->avdd_regulator)) {
		dev_err(dev, "Failed to get avdd-supply\n");
		return -EINVAL;
	}

	s5k3l6->iovdd_regulator = devm_regulator_get(dev, "iovdd");
	if (IS_ERR(s5k3l6->iovdd_regulator)) {
		dev_err(dev, "Failed to get iovdd-supply\n");
		return -EINVAL;
	}

	s5k3l6->dvdd_regulator = devm_regulator_get(dev, "dvdd");
	if (IS_ERR(s5k3l6->dvdd_regulator)) {
	...
	if (strcmp(s5k3l6->module_facing, "back") == 0)
		facing[0] = 'b';
	else
		facing[0] = 'f';
...
static const struct of_device_id s5k3l6_of_match[] = {
	{ .compatible = "quec,s5k3l6" },
	{},
};
MODULE_DEVICE_TABLE(of, s5k3l6_of_match);
#endif

static const struct i2c_device_id s5k3l6_match_id[] = {
	{ "quec,s5k3l6", 0 },
	{},
};
...
static struct i2c_driver s5k3l6_i2c_driver = {
	...
	.id_table	= s5k3l6_match_id,
};

static int __init sensor_mod_init(void)
{
	return i2c_add_driver(&s5k3l6_i2c_driver);
}

static void __exit sensor_mod_exit(void)
{
	i2c_del_driver(&s5k3l6_i2c_driver);
}

device_initcall_sync(sensor_mod_init);
module_exit(sensor_mod_exit);

MODULE_DESCRIPTION("quec s5k3l6 sensor driver");
