...
/* Pixel rate for 2-lane RAW10 with 344MHz link frequency. */
#define IMX219_PIXEL_RATE		137600000

#define IMX219_DEFAULT_LINK_FREQ	344000000

#define IMX219_DEFAULT_MODE_INDEX	2

#define IMX219_NAME			"imx219"

/* V_TIMING internal */
#define IMX219_REG_VTS			0x0160
#define IMX219_VTS_15FPS		0x09c1
#define IMX219_VTS_30FPS_1080P		0x06e3
#define IMX219_VTS_30FPS_BINNED		0x0534
...
#define IMX219_CHIP_ID_READ_RETRIES	5
...
static void imx219_set_default_format(struct imx219 *imx219)
{
	...
	fmt->width = supported_modes[IMX219_DEFAULT_MODE_INDEX].width;
	fmt->height = supported_modes[IMX219_DEFAULT_MODE_INDEX].height;
...
static int imx219_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	/* Initialize try_fmt */
	try_fmt->width = supported_modes[IMX219_DEFAULT_MODE_INDEX].width;
	try_fmt->height = supported_modes[IMX219_DEFAULT_MODE_INDEX].height;
				      ...
				      struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	mutex_lock(&imx219->mutex);
	fie->code = imx219_get_format_code(imx219, imx219->fmt.code);
	mutex_unlock(&imx219->mutex);

	fie->width = mode->width;
	fie->height = mode->height;
	fie->interval.numerator = IMX219_PPL_DEFAULT * mode->vts_def;
	fie->interval.denominator = IMX219_PIXEL_RATE;
				...
				struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_CROP_BOUNDS: {
		...
		/*
		 * Report bounds in active output coordinates to keep CIF crop checks
		 * consistent with get_fmt()/channel-info dimensions.
		 */
		mutex_lock(&imx219->mutex);
		sel->r.top = 0;
		sel->r.left = 0;
		sel->r.width = imx219->mode->width;
		sel->r.height = imx219->mode->height;
		mutex_unlock(&imx219->mutex);
...
static int imx219_identify_module(struct imx219 *imx219)
{
	...
	for (i = 0; i < IMX219_CHIP_ID_READ_RETRIES; i++) {
		ret = imx219_read_reg(imx219, IMX219_REG_CHIP_ID,
				      IMX219_REG_VALUE_16BIT, &val);
		if (!ret)
			break;

		/* Leave a short gap to tolerate late sensor wake-up. */
		usleep_range(3000, 4000);
	}
	if (ret) {
		dev_err(&client->dev,
			"failed to read chip id %x after %d retries\n",
			IMX219_CHIP_ID, IMX219_CHIP_ID_READ_RETRIES);
		return ret;
	}
				   ...
				   struct v4l2_subdev_frame_interval *fi)
{
	...
	fi->interval.numerator = IMX219_PPL_DEFAULT * mode->vts_def;
	fi->interval.denominator = IMX219_PIXEL_RATE;
...
static long imx219_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_GET_MODULE_INFO:
		...
		strscpy(inf->base.lens, imx219->lens_name,
	...
	case RKMODULE_GET_CHANNEL_INFO:
		...
		ch_info->bus_fmt = imx219_get_format_code(imx219,
							       imx219->fmt.code);
		mutex_unlock(&imx219->mutex);
...
static int imx219_probe(struct i2c_client *client)
{
	...
	imx219->module_index = module_index;
	...
	imx219->reset_gpio = devm_gpiod_get_optional(dev, "reset",
						     GPIOD_OUT_HIGH);
	imx219->pwdn_gpio = devm_gpiod_get_optional(dev, "pwdn",
						    GPIOD_OUT_HIGH);
	...
	/* Set default mode to the XML-validated 1640x1232 mode. */
	imx219->mode = &supported_modes[IMX219_DEFAULT_MODE_INDEX];
	...
	snprintf(imx219->sd.name, sizeof(imx219->sd.name), "m%02d_%s_%s %s",
		 imx219->module_index, facing, IMX219_NAME,
		 dev_name(dev));
