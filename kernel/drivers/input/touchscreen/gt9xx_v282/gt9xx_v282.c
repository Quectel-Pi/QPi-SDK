...
static const char *goodix_ts_name = "goodix-v282-ts";
static const char *goodix_input_phys = "input/ts";
struct i2c_client *i2c_connect_client_v282;
 ...
 */
int gtp_i2c_read_v282(struct i2c_client *client, u8 *buf, int len)
{
 ...
 *********************************************************/
int gtp_i2c_write_v282(struct i2c_client *client, u8 *buf, int len)
 ...
 *********************************************************/
s32 gtp_i2c_read_dbl_check_v282(struct i2c_client *client,
			   u16 addr, u8 *rxbuf, int len)
{
	...
	while (retry++ < 3) {
		...
		buf[0] = (u8)(addr >> 8);
		buf[1] = (u8)(addr & 0xFF);
		gtp_i2c_read_v282(client, buf, len + 2);

		memset(confirm_buf, 0xAB, 16);
		confirm_buf[0] = (u8)(addr >> 8);
		confirm_buf[1] = (u8)(addr & 0xFF);
		gtp_i2c_read_v282(client, confirm_buf, len + 2);
 ...
 *		< 0: failed
 *********************************************************/
s32 gtp_send_cfg_v282(struct i2c_client *client)
{
	s32 ret, i;
	u8 check_sum;
	s32 retry = 0;
	...
	for (retry = 0; retry < RETRY_MAX_TIMES; retry++) {
		ret = gtp_i2c_write_v282(client, cfg->data,
...
static int gtp_gesture_handler(struct goodix_ts_data *ts)
{
	...
	ret = gtp_i2c_read_v282(ts->client, doze_buf, 3);
	if (ret < 0) {
	...
	if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b') ||
		...
		/*  clear 0x814B */
		doze_buf[2] = 0x00;
		gtp_i2c_write_v282(ts->client, doze_buf, 3);
	} else {
		/*  clear 0x814B */
		doze_buf[2] = 0x00;
		gtp_i2c_write_v282(ts->client, doze_buf, 3);
			 ...
			 u8 *key_value)
{
	...
	ret = gtp_i2c_read_v282(ts->client, point_data, 12);
	if (ret < 0) {
	...
	if (touch_num > 1) {
		...
		ret = gtp_i2c_read_v282(ts->client, buf, 2 + 8 * (touch_num - 1));
		if (ret < 0) {
...
exit_get_point:
	if (!test_bit(RAW_DATA_MODE, &ts->flags)) {
		ret = gtp_i2c_write_v282(ts->client, end_cmd, 3);
		if (ret < 0)
			      ...
			      struct goodix_point_t *points)
{
	...
	struct goodix_point_t first_point;
	bool have_point = touch_num > 0;

	if (have_point) {
		first_point = points[0];
		input_report_key(ts->input_dev, BTN_TOUCH, 1);
		input_report_abs(ts->input_dev, ABS_X, first_point.x);
		input_report_abs(ts->input_dev, ABS_Y, first_point.y);
	}
	...
	for (i = 0; i < ts->pdata->max_touch_id; i++) {
		if (touch_num && i == points->id) {
			...
			/* Treat all contacts as finger to avoid tablet classification */
			       ...
			       struct goodix_point_t *points)
{
	...
	pre_touch = cur_touch;
	if (have_point) {
		/* Mirror first contact to classic ABS axes */
		input_report_abs(ts->input_dev, ABS_X, first_point.x);
		input_report_abs(ts->input_dev, ABS_Y, first_point.y);
	}
...
void gtp_int_sync_v282(struct goodix_ts_data *ts, s32 ms)
{
	if (!ts->pdata->int_sync)
		return;
 ...
 *	None.
 *******************************************************/
void gtp_reset_guitar_v282(struct i2c_client *client, s32 ms)
{
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
...
static int gtp_enter_doze(struct goodix_ts_data *ts)
{
	...
	while (retry++ < 5) {
		...
		ret = gtp_i2c_write_v282(ts->client, i2c_control_buf, 3);
		if (ret < 0) {
...
static int gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	while (retry++ < 10) {
		...
		if (!ret) {
			...
			gtp_int_sync_v282(ts, 25);
			if (ts->pdata->esd_protect)
				gtp_init_ext_watchdog(ts->client);

			return ret;
		}
		gtp_reset_guitar_v282(ts->client, 20);
	}
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
	...
	/* read sensor id */
	ret = gtp_i2c_read_dbl_check_v282(ts->client, GTP_REG_SENSOR_ID,
				     &sensor_id, 1);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (!ts->pdata->driver_send_cfg) {
		...
		ret = gtp_i2c_read_v282(ts->client,
				   cfg->data, cfg->length +
				   GTP_ADDR_LENGTH);
		if (ret < 0)
	...
	/* check firmware */
	ret = gtp_i2c_read_dbl_check_v282(ts->client, 0x41E4, opr_buf, 1);
	if (SUCCESS == ret) {
		if (opr_buf[0] != 0xBE) {
				    ...
				    size_t size, loff_t *ppos)
{
	...
	struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client_v282);
	struct goodix_config_data *cfg = &ts->pdata->config;
	...
	data_len += snprintf(ptr + data_len, 4096 - data_len,
			     "====real value====\n");
	ret = gtp_i2c_read_v282(i2c_connect_client_v282, temp_data,
				     ...
				     size_t count, loff_t *off)
{
	...
	ret = gtp_i2c_write_v282(ts->client, file_config, file_cfg_len + 2);
	if (ret > 0) {
				    ...
				    const char *buf, size_t count)
{
	...
	retval = gup_update_proc_v282(update_file_name);
	if (retval == FAIL)
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	gtp_reset_guitar_v282(data->client, 20);
...
s32 gtp_get_fw_info(struct i2c_client *client, struct goodix_fw_info *fw_info)
{
	...
	ret = gtp_i2c_read_v282(client, buf, sizeof(buf));
	if (ret < 0) {
	...
	ret = gtp_i2c_read_dbl_check_v282(client, GTP_REG_SENSOR_ID,
				     &fw_info->sensor_id, 1);
...
static int gtp_i2c_test(struct i2c_client *client)
{
	...
	while (retry++ < 3) {
		ret = gtp_i2c_read_v282(client, test, 3);
		if (ret == 2)
			return 0;
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	/* Mark as direct-touch so udev classifies it as touchscreen */
	__set_bit(INPUT_PROP_DIRECT, ts->input_dev->propbit);
	input_set_capability(ts->input_dev, EV_KEY, BTN_TOUCH);
	input_set_capability(ts->input_dev, EV_KEY, BTN_TOOL_FINGER);
				...
				struct goodix_ts_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "max_touch_num",
				   &pdata->max_touch_id);
	if (ret || pdata->max_touch_id > GTP_MAX_TOUCH_ID) {
		dev_info(dev, "Unset max_touch_num, use default\n");
		pdata->max_touch_id = GTP_MAX_TOUCH_ID;
	}

	ret = of_property_read_u32(np, "max-x",
				   &pdata->abs_size_x);
	if (ret) {
		dev_info(dev, "Unset max-x, use default\n");
		pdata->abs_size_x = GTP_DEFAULT_MAX_X;
	}

	ret = of_property_read_u32(np, "max-y",
				   &pdata->abs_size_y);
	if (ret) {
		dev_info(dev, "Unset max-y, use default\n");
		pdata->abs_size_y = GTP_DEFAULT_MAX_Y;
	}

	ret = of_property_read_u32(np, "max-w",
				   &pdata->max_touch_width);
	if (ret) {
		dev_info(dev, "Unset max-w, use default\n");
		pdata->max_touch_width = GTP_DEFAULT_MAX_WIDTH;
	}

	ret = of_property_read_u32(np, "max-p",
				   &pdata->max_touch_pressure);
	if (ret) {
		dev_info(dev, "Unset max-p, use default\n");
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	pdata->irq_gpio = of_get_named_gpio_flags(np, "touch-gpio", 0, (enum of_gpio_flags *)(&pdata->irq_flags));
    pdata->rst_gpio = of_get_named_gpio_flags(np, "reset-gpio", 0,&rst_flags);

    //pdata->pwr_pin = of_get_named_gpio_flags(np, "power-gpio", 0, &pwr_flags);
   /*
	pdata->irq_gpio = of_get_named_gpio(np, "touch-gpio", 0);
...
static int gtp_power_init(struct goodix_ts_data *ts)
{
    ...
    // 打印 regulator 的调试信息（需内核支持 DEBUG 或动态调试）
    pr_debug("regulator_get success, vdd=%p\n", ts->vdd_ana);
	}
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	i2c_connect_client_v282 = client;
...
static int gtp_gesture_wakeup(struct goodix_ts_data *ts)
{
	...
	do {
		gtp_reset_guitar_v282(ts->client, 10);
		ret = gtp_i2c_test(ts->client);
		if (!ret)
			break;
	} while (--retry);
...
static int gtp_init_ext_watchdog(struct i2c_client *client)
{
	...
	ret = gtp_i2c_write_v282(client, opr_buffer, 3);
	if (ret == 1)
		return 0;
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		ret = gtp_i2c_read_v282(ts->client, esd_buf, 4);
		if (ret < 0)
			continue;
		...
		if (esd_buf[2] == (u8)GTP_ESD_CHECK_VALUE ||
			...
			gtp_i2c_read_v282(ts->client, esd_buf, 4);
			if (ret < 0)
				continue;
			...
			gtp_i2c_write_v282(ts->client, esd_buf, 3);
			break;
		}
	}
	if (i >= 3) {
		...
		esd_buf[3] = 0x01;
		esd_buf[4] = 0x01;
		gtp_i2c_write_v282(ts->client, esd_buf, 5);
		...
		gtp_reset_guitar_v282(ts->client, 50);
		msleep(GTP_50_DLY_MS);
		gtp_send_cfg_v282(ts->client);
	}
