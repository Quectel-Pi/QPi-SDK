...
static s32 tool_i2c_read_no_extra(u8 *buf, u16 len)
{
	...
	for (i = 0; i < cmd_head.retry; i++) {
		ret = gtp_i2c_read_v282(ts->client, buf, len + GTP_ADDR_LENGTH);
		if (ret > 0)
			break;
	}
	return ret;
...
static s32 tool_i2c_write_no_extra(u8 *buf, u16 len)
{
	...
	for (i = 0; i < cmd_head.retry; i++) {
		ret = gtp_i2c_write_v282(ts->client, buf, len);
		if (ret > 0)
			break;
	}
			  ...
			  size_t len, loff_t *off)
{
	...
	if (1 == cmd_head.wr) {
		...
		gtp_reset_guitar_v282(gt_client, 20);
	}

	else if (11 == cmd_head.wr) {/*Enter update mode!*/
		if (FAIL == gup_enter_update_mode_v282(gt_client))
			return -EPERM;
	} else if (13 == cmd_head.wr) {/*Leave update mode!*/
		gup_leave_update_mode_v282(gt_client);
		...
		if (FAIL == gup_update_proc_v282((void *)cmd_head.data))
			return -EPERM;
	}
			 ...
			 size_t size, loff_t *ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		progress_buf[0] = show_len_v282 >> 8;
		progress_buf[1] = show_len_v282 & 0xff;
		progress_buf[2] = total_len_v282 >> 8;
		progress_buf[3] = total_len_v282 & 0xff;
