...
struct st_update_msg update_msg_v282;
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	ret = gtp_i2c_read_dbl_check_v282(ts->client, GTP_REG_CONFIG_DATA,
				     &opr_buf[0], 1);
	if (ret == SUCCESS) {
	...
	ret = gtp_send_cfg_v282(ts->client);
	if (ret < 0)
...
static u8 gup_get_ic_msg(struct i2c_client *client, u16 addr, u8 *msg, s32 len)
{
	...
	for (i = 0; i < 5; i++) {
		if (gtp_i2c_read_v282(client, msg, GTP_ADDR_LENGTH + len) > 0)
			break;
	}
...
static u8 gup_set_ic_msg(struct i2c_client *client, u16 addr, u8 val)
{
	...
	for (i = 0; i < 5; i++) {
		if (gtp_i2c_write_v282(client, msg, GTP_ADDR_LENGTH + 1) > 0)
			break;
	}
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
	...
	ret = gtp_i2c_read_dbl_check_v282(client, GUP_REG_HW_INFO,
	 ...
	 */
	for (i = 0; i < 4; i++)
		update_msg_v282.ic_fw_msg.hw_info[i] = buf[GTP_ADDR_LENGTH + 3 - i];
		dev_dbg(&client->dev,
			"IC Hardware info:%02x%02x%02x%02x",
			update_msg_v282.ic_fw_msg.hw_info[0],
			update_msg_v282.ic_fw_msg.hw_info[1],
			update_msg_v282.ic_fw_msg.hw_info[2],
			update_msg_v282.ic_fw_msg.hw_info[3]);
	...
	for (retry = 0; retry < 2; retry++) {
		...
		update_msg_v282.fw_damaged = buf[GTP_ADDR_LENGTH];
		if ((0xBE != update_msg_v282.fw_damaged) && (!retry)) {
	...
	dev_dbg(&client->dev,
		"IC force update flag:0x%x", update_msg_v282.fw_damaged);

	/*  step3:get pid & vid */
	ret = gtp_i2c_read_dbl_check_v282(client, GUP_REG_PID_VID,
	...
	memset(update_msg_v282.ic_fw_msg.pid, 0, sizeof(update_msg_v282.ic_fw_msg.pid));
	memcpy(update_msg_v282.ic_fw_msg.pid, &buf[GTP_ADDR_LENGTH], 4);
	dev_dbg(&client->dev, "IC Product id:%s", update_msg_v282.ic_fw_msg.pid);
	 ...
	 *|------9110-----9110----|
	 *|------9110P----9111----|*/
	if (update_msg_v282.ic_fw_msg.pid[0] != 0) {
		if (!memcmp(update_msg_v282.ic_fw_msg.pid, "9111", 4)) {
			dev_dbg(&client->dev, "IC Mapping Product id:%s",
					update_msg_v282.ic_fw_msg.pid);
			memcpy(update_msg_v282.ic_fw_msg.pid, "9110P", 5);
		}
	}

	update_msg_v282.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH + 4] +
				   (buf[GTP_ADDR_LENGTH + 5] << 8);
	dev_dbg(&client->dev, "IC version id:%04x", update_msg_v282.ic_fw_msg.vid);

	return SUCCESS;
}

s32 gup_enter_update_mode_v282(struct i2c_client *client)
{
	s32 ret = -1;
	s32 retry = 0;
	u8 rd_buf[3];
...
void gup_leave_update_mode_v282(struct i2c_client *client)
{
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
	...
	gtp_reset_guitar_v282(i2c_connect_client_v282, 20);
}
				 ...
				 struct st_fw_head *fw_head)
{
	...
	dev_info(&client->dev, "IC VID:%04x\n", update_msg_v282.ic_fw_msg.vid);

	if (!memcmp(fw_head->pid, "9158", 4) &&
	    !memcmp(update_msg_v282.ic_fw_msg.pid, "915S", 4)) {
		dev_info(&client->dev, "Update GT915S to GT9158 directly!");
		return SUCCESS;
	}
	/* First two conditions */
	if (!memcmp(fw_head->hw_info, update_msg_v282.ic_fw_msg.hw_info,
	sizeof(update_msg_v282.ic_fw_msg.hw_info))) {
		fw_len = 42 * 1024;
	} else {
	...
	if (update_msg_v282.fw_total_len != fw_len) {
		dev_err(&client->dev,
			"Inconsistent firmware size, Update aborted!");
		dev_err(&client->dev,
			" Default size: %d(%dK), actual size: %d(%dK)",
			fw_len, fw_len/1024, update_msg_v282.fw_total_len,
			update_msg_v282.fw_total_len/1024);
		return FAIL;
	}
	dev_info(&client->dev, "Firmware length:%d(%dK)",
		 update_msg_v282.fw_total_len,
		 update_msg_v282.fw_total_len/1024);

	if (update_msg_v282.fw_damaged != 0xBE) {
		dev_info(&client->dev, "FW chksum error,need enter update.");
		return SUCCESS;
	}

	/*  20130523 start */
	if (strlen(update_msg_v282.ic_fw_msg.pid) < 3) {
	...
	/* check pid legality */
	for (i = 0; i < 3; i++) {
		if (!isdigit(update_msg_v282.ic_fw_msg.pid[i])) {
			dev_info(&client->dev,
	...
	if (pid_cmp_len < strlen(update_msg_v282.ic_fw_msg.pid))
		pid_cmp_len = strlen(update_msg_v282.ic_fw_msg.pid);

	if ((!memcmp(fw_head->pid, update_msg_v282.ic_fw_msg.pid, pid_cmp_len)) ||
			(!memcmp(update_msg_v282.ic_fw_msg.pid, "91XX", 4)) ||
		...
		/* The third condition */
		if (fw_head->vid != update_msg_v282.ic_fw_msg.vid) {
...
static int gup_update_config(struct i2c_client *client)
{
	...
	i = 0;
	while (i++ < 5) {
		ret = gtp_i2c_write_v282(client, file_config, file_cfg_len + 2);
		if (ret > 0) {
				...
				struct st_fw_head *fw_head, u8 *path)
{
	...
	ret = request_firmware(&update_msg_v282.fw, path, &client->dev);
	if (ret < 0) {
	...
	dev_info(&client->dev, "FW File: %s size=%zu",
		 path, update_msg_v282.fw->size);
	update_msg_v282.fw_data = update_msg_v282.fw->data;
	update_msg_v282.fw_total_len = update_msg_v282.fw->size;

	if (update_msg_v282.fw_total_len <
	...
	update_msg_v282.fw_total_len -= FW_HEAD_LENGTH;

	dev_dbg(&client->dev, "Bin firmware actual size: %d(%dK)",
		update_msg_v282.fw_total_len, update_msg_v282.fw_total_len/1024);

	memcpy(fw_head, update_msg_v282.fw_data, FW_HEAD_LENGTH);

	/* check firmware legality */
	fw_checksum = 0;
	for (i = 0; i < update_msg_v282.fw_total_len; i += 2) {
		u16 temp;

		temp = (update_msg_v282.fw_data[FW_HEAD_LENGTH + i] << 8) +
			update_msg_v282.fw_data[FW_HEAD_LENGTH + i + 1];
		fw_checksum += temp;
	}
...
invalied_fw:
	update_msg_v282.fw_data = NULL;
	update_msg_v282.fw_total_len = 0;
	release_firmware(update_msg_v282.fw);
	return FAIL;
}
		...
		u16 start_addr, u16 total_length)
{
	...
	while (burn_length < total_length) {
		...
		for (retry = 0; retry < MAX_FRAME_CHECK_TIME; retry++) {
			...
			ret = gtp_i2c_read_v282(client, rd_buf,
...
{
	if (!update_msg_v282.fw_data ||
	    update_msg_v282.fw_total_len < offset + length) {
		dev_err(&i2c_connect_client_v282->dev,
	...
	if (SEEK_SET == set_or_end) {
		memcpy(buf, &update_msg_v282.fw_data[FW_HEAD_LENGTH + offset],
		       length);
	} else {
		/* seek end */
		memcpy(buf, &update_msg_v282.fw_data[update_msg_v282.fw_total_len +
		       FW_HEAD_LENGTH - offset], length);
	}
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
	...
	update_msg_v282.fw_burned_len += FW_DSP_ISP_LENGTH;
	dev_dbg(&client->dev, "[burn_dsp_isp]Burned length:%d",
			update_msg_v282.fw_burned_len);
	ret = SUCCESS;
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
	...
	update_msg_v282.fw_burned_len += (FW_SECTION_LENGTH*4);
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
	...
	update_msg_v282.fw_burned_len += FW_DSP_LENGTH;
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
	...
	update_msg_v282.fw_burned_len += FW_BOOT_LENGTH;
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
	...
	if (update_msg_v282.fw_burned_len >= update_msg_v282.fw_total_len) {
	 ...
	 */
	ret = gup_load_section_file(fw_boot_isp, (update_msg_v282.fw_burned_len - FW_DSP_ISP_LENGTH),
	...
	update_msg_v282.fw_burned_len += FW_BOOT_ISP_LENGTH;
	dev_dbg(&client->dev,
		"[burn_fw_boot_isp]Burned length:%d", update_msg_v282.fw_burned_len);
	ret = SUCCESS;
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
	...
	offset = update_msg_v282.fw_burned_len - FW_DSP_ISP_LENGTH;
	...
	update_msg_v282.fw_burned_len += FW_GLINK_LENGTH;
	dev_dbg(&client->dev,
		"[burn_fw_link]Burned length:%d", update_msg_v282.fw_burned_len);
	ret = SUCCESS;
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	if (start_index >= update_msg_v282.fw_total_len) {
		dev_dbg(&client->dev, "No need to upgrade the gwake code!");
		return SUCCESS;
	}
	/* start_index = update_msg_v282.fw_burned_len - FW_DSP_ISP_LENGTH; */
	dev_info(&client->dev,
	...
	/* update_msg_v282.fw_burned_len += FW_GWAKE_LENGTH; */
	dev_dbg(&client->dev,
		"[burn_fw_gwake]Burned length:%d", update_msg_v282.fw_burned_len);
	ret = SUCCESS;
...
s32 gup_update_proc_v282(void *dir)
{
	s32 ret = 0;
	s32 update_ret = FAIL;
	u8	retry = 0;
	...
	ts = i2c_get_clientdata(i2c_connect_client_v282);
	...
	ret = gup_get_update_file(i2c_connect_client_v282, &fw_head, (u8 *)dir);
	if (FAIL == ret) {
		dev_err(&ts->client->dev,
	...
	ret = gup_get_ic_fw_msg(i2c_connect_client_v282);
	if (FAIL == ret) {
	...
	if (ts->force_update || dir) {
		...
		ret = gup_enter_update_judge(i2c_connect_client_v282, &fw_head);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
	...
	ret = gup_enter_update_mode_v282(ts->client);
	if (FAIL == ret) {
		dev_err(&ts->client->dev,
	...
	while (retry++ < 5) {
		show_len_v282 = 10;
		total_len_v282 = 100;
		update_msg_v282.fw_burned_len = 0;
		ret = gup_burn_dsp_isp(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
		...
		show_len_v282 = 20;
		ret = gup_burn_fw_gwake(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
		...
		show_len_v282 = 30;
		ret = gup_burn_fw_ss51(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
		...
		show_len_v282 = 40;
		ret = gup_burn_fw_dsp(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
		...
		show_len_v282 = 50;
		ret = gup_burn_fw_boot(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
		...
		ret = gup_burn_fw_boot_isp(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
		...
		show_len_v282 = 70;
		ret = gup_burn_fw_link(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
		...
		show_len_v282 = 80;
		ret = gup_burn_fw_finish(i2c_connect_client_v282);
		if (FAIL == ret) {
			dev_err(&ts->client->dev,
...
update_fail:
	...
	gup_leave_update_mode_v282(i2c_connect_client_v282);
...
u8 gup_init_update_proc(struct goodix_ts_data *ts)
{
	...
	thread = kthread_run(gup_update_proc_v282,
