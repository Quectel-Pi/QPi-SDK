...
static int rockchip_combphy_init(struct phy *phy)
{
	...
	/*
	 * 诊断：打印 phy_init 时刻（set_mode 前）的 PLL/接收端寄存器值。
	 * 冷启动首次 init 时为真正复位默认值；热切换（USB1->PCIE1）时
	 * 若 phy_exit 的 reset 不能清除配置寄存器，这里读到的是残留值。
	 */
	dev_info(priv->dev,
		 "combphy init raw: 0x14=0x%x 0x44=0x%x 0x38=0x%x 0x50=0x%x "
		 "0x28=0x%x 0x2c=0x%x 0x30=0x%x 0x34=0x%x 0x1b=0x%x 0x80=0x%x\n",
		 readl(priv->mmio + (0x5 << 2)),
		 readl(priv->mmio + (0x11 << 2)),
		 readl(priv->mmio + (0xe << 2)),
		 readl(priv->mmio + (0x14 << 2)),
		 readl(priv->mmio + (0xa << 2)),
		 readl(priv->mmio + (0xb << 2)),
		 readl(priv->mmio + (0xc << 2)),
		 readl(priv->mmio + (0xd << 2)),
		 readl(priv->mmio + (0x1b << 2)),
		 readl(priv->mmio + (0x20 << 2)));
...
static int rockchip_combphy_set_mode_callback(struct phy *phy,
					      enum phy_mode mode, int submode)
{
	...
	case PHY_MODE_SATA:
		...
		break;
	default:
		dev_dbg(priv->dev, "ignore unsupported phy mode %d\n", mode);
		break;
	}
...
static const struct phy_ops rockchip_combphy_ops = {
	...
	.set_mode = rockchip_combphy_set_mode_callback,
...
static int rk3576_combphy_cfg(struct rockchip_combphy_priv *priv)
{
	...
	case 100000000:
		...
		if (priv->mode == PHY_TYPE_PCIE) {
			...
			/*
			 * USB1 -> PCIE1 热切换修复（与 SATA0/PCIE0 残留问题同源）:
			 * USB3 模式会写 0x38(CTLE)/0x50(squelch)/0x14(input div)/
			 * 0x44(loop div)，而 phy_exit 的 reset 与 combphy rebind 均
			 * 不能清除这些 PLL/接收端配置寄存器（SATA 修复已证实复位
			 * 不清 PLL 配置）。从 USB1 热切换到 PCIE1 时残留 USB3 值
			 * 导致 PCIe Rx 信号处理异常，NVMe 无法 link up。
			 * 此处显式写回 PCIe 复位默认值（与冷启动一致的配置）。
			 */
			dev_info(priv->dev,
				 "rk3576 combphy PCIE cfg pre: 0x14=0x%x 0x44=0x%x 0x38=0x%x 0x50=0x%x\n",
				 readl(priv->mmio + (0x5 << 2)),
				 readl(priv->mmio + (0x11 << 2)),
				 readl(priv->mmio + (0xe << 2)),
				 readl(priv->mmio + (0x14 << 2)));

			/* PLL input clock divider ÷1（复位默认 0x0，USB3 模式写 ÷2=0x40 需恢复） */
			rockchip_combphy_updatel(priv, GENMASK(7, 6), 0x0, 0x14);
			/* PLL loop divider（复位默认 0x32，与 USB3 一致） */
			writel(0x32, priv->mmio + (0x11 << 2));
			/* 关闭 USB3 adaptive CTLE，PCIe Rx 使用自身 rx_trim 均衡 */
			rockchip_combphy_updatel(priv, GENMASK(0, 0), 0x0, 0x38);
			/* Rx squelch input filler bandwidth 恢复 PCIe 复位默认 0xb */
			writel(0x0b, priv->mmio + (0x14 << 2));

			dev_info(priv->dev,
				 "rk3576 combphy PCIE cfg post: 0x14=0x%x 0x44=0x%x "
				 "0x38=0x%x 0x50=0x%x 0x28=0x%x 0x2c=0x%x 0x30=0x%x "
				 "0x34=0x%x 0x1b=0x%x 0x80=0x%x\n",
				 readl(priv->mmio + (0x5 << 2)),
				 readl(priv->mmio + (0x11 << 2)),
				 readl(priv->mmio + (0xe << 2)),
				 readl(priv->mmio + (0x14 << 2)),
			...
			/*
			 * Set PLL loop divider / su_trim explicitly.
			 * PCIe 模式会写 0xa/0xb/0xd 为 PCIe 的 su_trim 值；
			 * 从 PCIe 热切换到 SATA 时这些寄存器残留 PCIe 值，
			 * 若不覆盖，PLL 仍按 PCIe 配置锁定，SATA OOB 协商失败
			 * （SStatus=1 SPD=0）。此处写回 SATA 复位默认值。
			 */
			writel(0x93, priv->mmio + (0xa << 2));
			writel(0x02, priv->mmio + (0xb << 2));
			writel(0x59, priv->mmio + (0xd << 2));
		}
		break;
	default:
		dev_err(priv->dev, "Unsupported rate: %lu\n", rate);
