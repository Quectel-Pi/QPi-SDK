...
struct rockchip_udphy {
	...
	struct gpio_desc *ssmux_gpio;
	int factory_u3_path; /* 0 => SEL=0 + lane <2 3>, 1 => SEL=1 + lane <0 1> */
};
...
static int udphy_factory_apply_path(struct rockchip_udphy *udphy, int path)
{
	u8 active_mode;
	int ret = 0;
	...
	udphy->factory_u3_path = path ? 1 : 0;

	if (udphy->ssmux_gpio)
		gpiod_set_value_cansleep(udphy->ssmux_gpio,
					 udphy->factory_u3_path);

	if (udphy->factory_u3_path) {
		udphy->flip = true;
		udphy->dp_lane_sel[0] = 0;
		udphy->dp_lane_sel[1] = 1;
		udphy->dp_lane_sel[2] = 3;
	...
	udphy->mode = UDPHY_MODE_DP_USB;
	udphy->dp_lanes = 2;
	active_mode = udphy->status;

	if (active_mode == UDPHY_MODE_NONE) {
		ret = udphy_setup(udphy);
		if (!ret)
			udphy->status = UDPHY_MODE_USB;
	} else {
		udphy->mode_change = true;
		ret = udphy_power_on(udphy, active_mode);
	}
...
int __maybe_unused rockchip_udphy_set_factory_u3_path(int id, int path)
{
	struct device *dev;
	...
	dev = bus_find_device_by_name(&platform_bus_type, NULL, "2b010000.phy");
	if (!dev)
		return -ENODEV;

	pdev = to_platform_device(dev);
	udphy = platform_get_drvdata(pdev);
	if (!udphy) {
		put_device(dev);
		return -ENODEV;
	}
	...
	ret = udphy_factory_apply_path(udphy, path);
	put_device(dev);

	return ret;
}
EXPORT_SYMBOL_GPL(rockchip_udphy_set_factory_u3_path);

static ssize_t factory_u3_path_show(struct device *dev,
				    struct device_attribute *attr,
				    char *buf)
{
	struct rockchip_udphy *udphy = dev_get_drvdata(dev);

	return sysfs_emit(buf, "%d\n", udphy->factory_u3_path ? 1 : 0);
}

static ssize_t factory_u3_path_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	...
	ret = udphy_factory_apply_path(udphy, val);
	if (ret)
		return ret;
...
static DEVICE_ATTR_RW(factory_u3_path);
...
static int rockchip_udphy_probe(struct platform_device *pdev)
{
	...
	udphy->ssmux_gpio = devm_gpiod_get_optional(dev, "ssmux", GPIOD_OUT_LOW);
	if (IS_ERR(udphy->ssmux_gpio))
		return PTR_ERR(udphy->ssmux_gpio);
	...
	ret = device_create_file(dev, &dev_attr_factory_u3_path);
	if (ret) {
		dev_err(dev, "failed to create factory_u3_path sysfs node\n");
		goto put_child;
	}
