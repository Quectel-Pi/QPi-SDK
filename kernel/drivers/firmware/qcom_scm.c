...
/**
 * qcom_scm_is_dump_enabled() - Check whether dump/download mode is enabled
 *
 * Return: true if download_mode is not NODUMP (off), i.e. any of
 * full/mini/both is active; false when dump is disabled.
 */
bool qcom_scm_is_dump_enabled(void)
{
	return download_mode != QCOM_DLOAD_NODUMP;
}
EXPORT_SYMBOL_GPL(qcom_scm_is_dump_enabled);
