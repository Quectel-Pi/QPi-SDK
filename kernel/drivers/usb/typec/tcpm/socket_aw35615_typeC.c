...
#include "socket_aw35615_typeC.h"

#define AW35615_TCPCI_DRIVER_VERSION		"V1.1.0"
static void aw35615_typea_set_path(struct aw35615_chip *chip,
				   enum aw35615_typea_path mode);
extern int rockchip_udphy_set_factory_u3_path(int id, int path);
...
static int tcpm_set_cc(struct tcpc_dev *dev, enum typec_cc_status cc)
{
	...
	case TYPEC_CC_RD:
		...
		if (chip->cc_polarity == TYPEC_POLARITY_CC1)
			aw35615_typea_set_path(chip, AW35615_TYPEA_PATH_DEVICE_CC1);
		else
			aw35615_typea_set_path(chip, AW35615_TYPEA_PATH_DEVICE_CC2);
		break;
	...
	case TYPEC_CC_RP_3_0:
		switches0_data |= (chip->cc_polarity == TYPEC_POLARITY_CC1) ?
				  ...
				  AW_REG_SWITCHES0_CC2_PU_EN;
		if (chip->cc_polarity == TYPEC_POLARITY_CC1)
			aw35615_typea_set_path(chip, AW35615_TYPEA_PATH_HOST_CC1);
		else
			aw35615_typea_set_path(chip, AW35615_TYPEA_PATH_HOST_CC2);
		break;
	default:
...
static int tcpm_set_vbus(struct tcpc_dev *dev, bool on, bool charge)
{
	...
	if (on) {
		if (chip->vbus_enabled_by_tcpm) {
			aw35615_log(chip, "vbus is already enabled by tcpm");
		} else {
			ret = regulator_enable(chip->vbus);
			if (ret < 0) {
				aw35615_log(chip,
					    "cannot enable vbus regulator, ret=%d",
					    ret);
				goto done;
			}

			chip->vbus_enabled_by_tcpm = true;
			aw35615_log(chip, "vbus regulator enable by tcpm");
		}

		if (!chip->usb0_host_path0_applied) {
			path_ret = rockchip_udphy_set_factory_u3_path(0, 0);
			if (path_ret < 0) {
				aw35615_log(chip,
					    "default host path0 apply failed, ret=%d",
					    path_ret);
			} else {
				chip->usb0_host_path0_applied = true;
				aw35615_log(chip,
					    "default host path0(A-B) applied");
			}
		}
		...
		if (chip->vbus_enabled_by_tcpm) {
			...
			if (ret < 0) {
				aw35615_log(chip,
					    "cannot disable vbus regulator, ret=%d",
					    ret);
				goto done;
			}

			chip->vbus_enabled_by_tcpm = false;
			aw35615_log(chip, "vbus regulator disable by tcpm");
		} else {
			aw35615_log(chip,
				    "skip vbus disable: not enabled by tcpm");
		}

		chip->vbus_on = false;
		chip->usb0_host_path0_applied = false;
	}
...
{
	int pupdctl1 = 0;
	int pupdctl2 = 0; /* pre-arm host path */
	int gpio2d7 = 0;

	switch (mode) {
	case AW35615_TYPEA_PATH_DEVICE_CC1:
		pupdctl1 = 1;
		pupdctl2 = 0;
		gpio2d7 = 1;
		break;

	case AW35615_TYPEA_PATH_DEVICE_CC2:
		pupdctl1 = 0;
		pupdctl2 = 0;
		gpio2d7 = 0;
		break;

	case AW35615_TYPEA_PATH_HOST_CC1:
		pupdctl1 = 0;
		pupdctl2 = 0;
		gpio2d7 = 1;
		break;

	case AW35615_TYPEA_PATH_HOST_CC2:
		pupdctl1 = 0;
		pupdctl2 = 0;
		gpio2d7 = 0;
		break;

	case AW35615_TYPEA_PATH_FLOAT:
	case AW35615_TYPEA_PATH_SAFE:
	default:
		pupdctl1 = 0;
		pupdctl2 = 1;
		gpio2d7 = 0;
	...
	if (chip->pupdctl1_gpiod)
		gpiod_set_value_cansleep(chip->pupdctl1_gpiod, pupdctl1);
	if (chip->pupdctl2_gpiod)
		gpiod_set_value_cansleep(chip->pupdctl2_gpiod, pupdctl2);
	if (chip->gpio2d7_gpiod)
		gpiod_set_value_cansleep(chip->gpio2d7_gpiod, gpio2d7);

	aw35615_log(chip, "typea_path mode=%d pupdctl1=%d pupdctl2=%d gpio2d7=%d",
		    mode, pupdctl1, pupdctl2, gpio2d7);
}

static int aw35615_typea_path_from_str(const char *buf,
				       enum aw35615_typea_path *path)
{
	if (sysfs_streq(buf, "float")) {
		*path = AW35615_TYPEA_PATH_FLOAT;
		return 0;
	}
	if (sysfs_streq(buf, "device_cc1")) {
		*path = AW35615_TYPEA_PATH_DEVICE_CC1;
		return 0;
	}
	if (sysfs_streq(buf, "device_cc2")) {
		*path = AW35615_TYPEA_PATH_DEVICE_CC2;
		return 0;
	}
	if (sysfs_streq(buf, "host_cc1")) {
		*path = AW35615_TYPEA_PATH_HOST_CC1;
		return 0;
	}
	if (sysfs_streq(buf, "host_cc2")) {
		*path = AW35615_TYPEA_PATH_HOST_CC2;
		return 0;
	}
...
static ssize_t aw35615_ft_override_write(struct file *file,
					 const char __user *ubuf,
					 size_t cnt, loff_t *ppos)
{
	struct aw35615_chip *chip = file->private_data;
	char buf[16];
	long val;
	...
	mutex_lock(&chip->lock);
	chip->factory_override = !!val;
	aw35615_log(chip, "factory_override := %d", chip->factory_override);
	mutex_unlock(&chip->lock);

	return cnt;
}

static int aw35615_ft_override_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}

static const struct file_operations aw35615_ft_override_fops = {
	.owner = THIS_MODULE,
	.open = aw35615_ft_override_open,
	.write = aw35615_ft_override_write,
	.llseek = no_llseek,
};

static ssize_t aw35615_ft_path_write(struct file *file,
				     const char __user *ubuf,
				     size_t cnt, loff_t *ppos)
{
	struct aw35615_chip *chip = file->private_data;
	...
	ret = aw35615_typea_path_from_str(buf, &path);
	if (ret)
		return ret;

	mutex_lock(&chip->lock);
	aw35615_typea_set_path(chip, path);
	mutex_unlock(&chip->lock);

	return cnt;
}

static int aw35615_ft_path_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}

static const struct file_operations aw35615_ft_path_fops = {
	.owner = THIS_MODULE,
	.open = aw35615_ft_path_open,
	.write = aw35615_ft_path_write,
	.llseek = no_llseek,
};
		...
		const struct i2c_device_id *id)
{
	...
	chip->pupdctl1_gpiod = devm_gpiod_get_optional(dev, "pupdctl1",
					       GPIOD_OUT_LOW);
	if (IS_ERR(chip->pupdctl1_gpiod))
		return PTR_ERR(chip->pupdctl1_gpiod);

	chip->pupdctl2_gpiod = devm_gpiod_get_optional(dev, "pupdctl2",
					       GPIOD_OUT_LOW);
	if (IS_ERR(chip->pupdctl2_gpiod))
		return PTR_ERR(chip->pupdctl2_gpiod);

	chip->gpio2d7_gpiod = devm_gpiod_get_optional(dev, "gpio2d7",
					      GPIOD_OUT_LOW);
	if (IS_ERR(chip->gpio2d7_gpiod))
		return PTR_ERR(chip->gpio2d7_gpiod);
	...
	chip->vbus_enabled_by_tcpm = false;
	chip->usb0_host_path0_applied = false;
	chip->factory_override = false;

	snprintf(override_name, sizeof(override_name), "aw35615-ft-override-%s", dev_name(chip->dev));
	chip->ft_override_dentry = debugfs_create_file(override_name, 0600, usb_debug_root,
						       chip, &aw35615_ft_override_fops);

	snprintf(override_name, sizeof(override_name), "aw35615-ft-path-%s", dev_name(chip->dev));
	chip->ft_path_dentry = debugfs_create_file(override_name, 0600, usb_debug_root,
						   chip, &aw35615_ft_path_fops);
	...
	dev_info(dev, "socket_aw35615_typeC probe success\n");
...
static void aw35615_remove(struct i2c_client *client)
{
	...
	destroy_workqueue(chip->wq);
	aw35615_debugfs_exit(chip);
	debugfs_remove(chip->ft_override_dentry);
	debugfs_remove(chip->ft_path_dentry);
...
static int aw35615_pm_resume(struct device *dev)
{
    ...
    ret = tcpm_init(&chip->tcpc_dev);
    if (ret < 0){
		dev_err(dev, "Resume: Failed to re-init tcpci: %d\n", ret);
	}
    else{
		dev_info(dev, "Resume: tcpm_init success");
	}
        
    if (chip->toggling_mode != TOGGLING_MODE_OFF) {
        ret = aw35615_set_toggling(chip, chip->toggling_mode);
        if (ret < 0){
			dev_err(dev, "Resume: Failed to restore toggling: %d\n", ret);
		}    
    }
