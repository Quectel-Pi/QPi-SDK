...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	mux_gpio = devm_gpiod_get_optional(dev, "mux",
					   GPIOD_ASIS | GPIOD_FLAGS_BIT_NONEXCLUSIVE);
	if (IS_ERR(mux_gpio))
		return dev_err_probe(dev, PTR_ERR(mux_gpio),
				     "failed to get mux-gpios\n");

	if (mux_gpio && gpiod_get_value(mux_gpio)) {
		dev_info(dev, "skip probe: USB/PCIE mux is in PCIe mode\n");
		return -ENODEV;
	}
...
static void dwc3_shutdown(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(dev->of_node, "rockchip,rk3576-dwc3")) {
		/*
		 * 关机/重启时先优雅断开 USB 设备，再关闭控制器。
		 *
		 * 不要直接调用 dwc3_remove()：它会立即把控制器连根移除
		 * （含 pm_runtime 状态切换、driver data 清空）。此时若 USB
		 * 存储设备尚未被断开，SCSI 层收尾命令（SYNCHRONIZE CACHE /
		 * TEST UNIT READY）会提交到已死的 USB 总线，UAS 驱动把
		 * -ENOENT(-2) 误判为"设备忙"，在 uas_do_work() 中无限重试，
		 * 导致关机时刷屏卡死（"sense submit err -2" 循环）。
		 *
		 * 按"先断设备、再关控制器"的顺序：
		 *  1. dwc3_core_exit_mode(): 移除 xhci（触发 usb_disconnect
		 *     -> usb-storage/uas disconnect 干净收尾），同时退出
		 *     gadget/DRD 模式
		 *  2. dwc3_core_exit(): 关 PHY/时钟/复位，保证 warm reboot
		 *     后 dwc3 状态干净（保留该特判的原始目的）
		 *
		 * 不做 dwc3_remove() 中的 pm_runtime 状态清理与
		 * platform_set_drvdata(NULL)：shutdown 后系统立即重启/关机，
		 * 无需归还 PM 状态，且避免其与 xhci 移除过程产生竞态。
		 */
		dwc3_core_exit_mode(dwc);
		dwc3_core_exit(dwc);
		dwc3_ulpi_exit(dwc);
