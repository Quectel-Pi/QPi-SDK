...
struct rk_pcie {
	...
	struct gpio_desc		*pwkey_gpio;
	struct gpio_desc		*modem_rst_gpio;
	u32				perst_inactive_ms;
	u32				s2r_perst_inactive_ms;
	u32				modem_power_delay_ms;
	u32				modem_reset_pulse_ms;
...
static int rk_pcie_disable_power(struct rk_pcie *rk_pcie)
{
	...
	if (rk_pcie->modem_rst_gpio)
		gpiod_set_value_cansleep(rk_pcie->modem_rst_gpio, 0);

	if (rk_pcie->pwkey_gpio)
		gpiod_set_value_cansleep(rk_pcie->pwkey_gpio, 0);
...
static void rk_pcie_prepare_modem(struct rk_pcie *rk_pcie)
{
	struct device *dev = rk_pcie->pci->dev;

	if (rk_pcie->pwkey_gpio) {
		msleep(100);
		gpiod_set_value_cansleep(rk_pcie->pwkey_gpio, 1);
		dev_info(dev, "5G HAT power key asserted\n");
	}

	if (rk_pcie->modem_rst_gpio) {
		gpiod_set_value_cansleep(rk_pcie->modem_rst_gpio, 1);
		msleep(rk_pcie->modem_reset_pulse_ms);
		gpiod_set_value_cansleep(rk_pcie->modem_rst_gpio, 0);
		dev_info(dev, "5G HAT reset pulse %ums\n",
			 rk_pcie->modem_reset_pulse_ms);
	}

	if (rk_pcie->modem_power_delay_ms)
		msleep(rk_pcie->modem_power_delay_ms);
}
					 ...
					 struct rk_pcie *rk_pcie)
{
	...
	rk_pcie->pwkey_gpio = devm_gpiod_get_optional(&pdev->dev, "pwkey",
						       GPIOD_OUT_LOW);
	if (IS_ERR(rk_pcie->pwkey_gpio)) {
		dev_err(&pdev->dev, "invalid pwkey-gpios property in node\n");
		return PTR_ERR(rk_pcie->pwkey_gpio);
	}

	rk_pcie->modem_rst_gpio = devm_gpiod_get_optional(&pdev->dev,
							  "modem-reset", GPIOD_OUT_LOW);
	if (IS_ERR(rk_pcie->modem_rst_gpio)) {
		dev_err(&pdev->dev, "invalid modem-reset-gpios property in node\n");
		return PTR_ERR(rk_pcie->modem_rst_gpio);
	}
	...
	device_property_read_u32(&pdev->dev, "delay-ms",
				     &rk_pcie->modem_power_delay_ms);

	if (device_property_read_u32(&pdev->dev, "modem-reset-pulse-ms",
				     &rk_pcie->modem_reset_pulse_ms))
		rk_pcie->modem_reset_pulse_ms = 500;
...
static int rk_pcie_mux_check(struct device *dev)
{
	struct device_node *mux_np;
	int mux_val;

	if (!device_property_present(dev, "rockchip,mux-node"))
		return 0;

	mux_np = of_parse_phandle(dev->of_node, "rockchip,mux-node", 0);
	if (!mux_np)
		return dev_err_probe(dev, -EINVAL,
				     "invalid rockchip,mux-node\n");

	mux_val = rk3576_usb_pcie_mux_get_mode(mux_np);
	of_node_put(mux_np);
	if (mux_val < 0)
		return dev_err_probe(dev, mux_val, "mux is not ready\n");

	if (device_property_read_bool(dev, "rockchip,mux-skip-when-high")) {
		if (mux_val) {
			dev_info(dev, "skip probe: mux is in SATA mode\n");
			return -ENODEV;
		}
	} else if (!mux_val) {
		dev_info(dev, "skip probe: mux is in USB mode\n");
		return -ENODEV;
	}
...
static int rk_pcie_really_probe(void *p)
{
	...
	/*
	 * MUX gating: skip probe when the external mux is not routed to this
	 * controller.
	 *  - usb-pcie-mux (PCIE1): low = USB, high = PCIE
	 *    skip when mux pin is low.
	 *  - sata-pcie-mux (PCIE0): low = PCIE, high = SATA
	 *    skip when mux pin is high (DTS: rockchip,mux-skip-when-high).
	 *
	 * We query the mux driver's software mode via rockchip,mux-node.
	 * The mux GPIO itself is owned by rk3576-usb-pcie-mux; we must NOT
	 * hold a (NONEXCLUSIVE) gpiod reference here, because devm would
	 * free the owner's GPIO request on unbind and break re-probe.
	 */
	ret = rk_pcie_mux_check(dev);
	if (ret)
		return ret;
...
static int rk_pcie_probe(struct platform_device *pdev)
{
	...
	/*
	 * Do the mux check synchronously.  With threaded PCIe init, returning
	 * -EPROBE_DEFER from the worker is too late because the platform probe
	 * has already returned success to the driver core.
	 */
	ret = rk_pcie_mux_check(&pdev->dev);
	if (ret)
		return ret;
...
static int rk_pcie_remove(struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_PCIE_RK_THREADED_INIT)) {
		...
		start = jiffies;
		while ((rk_pcie && !rk_pcie->finish_probe) && time_before(jiffies, start + timeout)) {
			set_current_state(TASK_INTERRUPTIBLE);
			schedule_timeout(msecs_to_jiffies(200));
		};
