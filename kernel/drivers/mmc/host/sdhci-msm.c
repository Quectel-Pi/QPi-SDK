 ...
 */
 ...
 static int sdhci_msm_probe(struct platform_device *pdev)
 {
	 ...
	 struct gpio_desc *wifi_en_gpiod = devm_gpiod_get_optional(&pdev->dev, "quec,wifi-en", GPIOD_OUT_HIGH);
	 if (IS_ERR(wifi_en_gpiod)) {
		 dev_err_probe(&pdev->dev, PTR_ERR(wifi_en_gpiod),
			 "get quec,wifi-en-gpio failed\n");
	 }
