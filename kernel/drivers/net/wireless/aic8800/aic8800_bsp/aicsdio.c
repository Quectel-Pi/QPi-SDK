...
static bool aicbsp_sdio_driver_registered;
...
#endif /* CONFIG_PLATFORM_ROCKCHIP || CONFIG_PLATFORM_ROCKCHIP2 */
...
int aicbsp_sdio_init(void)
{
	...
	if (aicbsp_sdio_driver_registered)
		return aicbsp_sdiodev ? 0 : -ENODEV;

	if (sdio_register_driver(&aicbsp_sdio_driver)) {
		return -1;
	} else {
		aicbsp_sdio_driver_registered = true;
	}
#if defined(CONFIG_PLATFORM_ROCKCHIP) || defined(CONFIG_PLATFORM_ROCKCHIP2)
...
void aicbsp_sdio_exit(void)
{
	if (aicbsp_sdio_driver_registered) {
		...
		aicbsp_sdio_driver_registered = false;
	}
}
