...
{
	u32 cnt = 0, err_det_cnt = 0;
	u32 write_val = 0, val_270 = 0;
...
CR270_W_IN_W:
	...
	while (1) {
		val_270 = MAC_REG_R32(R_AX_WLAN_XTAL_SI_CTRL);
		/*printk("In %s while loop, CR0x270=0x%x\n", __func__, val_270);*/
		if ((val_270 & 0xFF) != offset) {
			printk("%s: ERROR detected: CR270 offset(0x%x) != API input offset(0x%x)\n",
					__func__, val_270, offset);
			err_det_cnt--;
			if (!err_det_cnt) {
				printk("ERROR detected cnt up to 10, break!\n");
				break;
			}
			goto CR270_W_IN_W;
		}
		if (!(val_270 & B_AX_WL_XTAL_SI_CMD_POLL))
			break;
...
{
	u32 cnt = 0, err_det_cnt = 0;
	u32 write_val = 0, val_270 = 0, val32_tmp1 = 0, val32_tmp2 = 0;
...
#if 0
			val32_tmp2 = MAC_REG_R32(0x3F0);
			printk("In %s while loop, CR0x270=0x%x, CR0x3F0=0x%x\n", __func__, val_270, val32_tmp2);
#endif
