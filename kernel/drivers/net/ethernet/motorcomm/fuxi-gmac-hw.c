...
static void fxgmac_enable_channel_rx(struct fxgmac_pdata* pdata, unsigned int queue)
{
    ...
    /* Enable MAC Rx */
    regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
    if (!FXGMAC_GET_REG_BITS(regval, MAC_CR_CST_POS, MAC_CR_CST_LEN) &&
        !FXGMAC_GET_REG_BITS(regval, MAC_CR_RE_POS, MAC_CR_RE_LEN))
    {
        regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_CST_POS,
            MAC_CR_CST_LEN, 1);
        regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_ACS_POS,
...
static void fxgmac_tx_mmc_int(struct fxgmac_pdata *pdata)
{
    ...
    stats->txerrors = stats->txunderflowerror + stats->txlatecollisionframes +
                    stats->txexcessivecollisionframes +
                   stats->txcarriererrorframes + stats->txexcessivedeferralerror;
}
...
static void fxgmac_rx_mmc_int(struct fxgmac_pdata *pdata)
{
    ...
    stats->rxerrors = stats->rxcrcerror + stats->rxalignerror +
              stats->rxrunterror + stats->rxjabbererror + stats->rxlengtherror +
              stats->rxwatchdogerror + stats->rxreceiveerrorframe;
}
...
static void fxgmac_read_mmc_stats(struct fxgmac_pdata *pdata)
{
    ...
    stats->rxerrors = stats->rxcrcerror + stats->rxalignerror +
                       stats->rxrunterror + stats->rxjabbererror +
                       stats->rxlengtherror + stats->rxwatchdogerror +
                       stats->rxreceiveerrorframe;
}
...
static void fxgmac_phy_downgrade_feature(struct fxgmac_pdata* pdata, u32 enable)
{
    u32 regval = 0;

    fxgmac_read_ephy_reg(pdata, REG_MII_DOWNG_CTRL, &regval);
    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_DOWNG_CTRL_DOWN_GRADE_POS,
                                 REG_MII_DOWNG_CTRL_DOWN_GRADE_LEN, enable);
    fxgmac_write_ephy_reg(pdata, REG_MII_DOWNG_CTRL, regval);

    //after change SmartSpeed need to soft reset
    fxgmac_read_ephy_reg(pdata, REG_MII_BMCR, &regval);
    regval = FXGMAC_SET_REG_BITS(regval, PHY_CR_RESET_POS, PHY_CR_RESET_LEN, 1);
    fxgmac_write_ephy_reg(pdata, REG_MII_BMCR, regval);
}

static void fxgmac_phy_downgrade_retry_Limit(struct fxgmac_pdata* pdata, u32 limit)
{
    u32 regval = 0;

    fxgmac_read_ephy_reg(pdata, REG_MII_DOWNG_CTRL, &regval);
    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_DOWNG_CTRL_RETRY_LIMIT_POS,
                                 REG_MII_DOWNG_CTRL_RETRY_LIMIT_LEN, limit);
    fxgmac_write_ephy_reg(pdata, REG_MII_DOWNG_CTRL, regval);
...
static void fxgmac_phy_fast_linkdown_feature(struct fxgmac_pdata* pdata, u32 enable)
{
    u32 regval;

    fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR, REG_MII_EXT_EEE_CONTROL2);
    fxgmac_read_ephy_reg(pdata, REG_MII_EXT_DATA, &regval);

    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_EXT_EEE_CONTROL2_BYPASS_FAST_LINKDOWN_POS,
                               REG_MII_EXT_EEE_CONTROL2_BYPASS_FAST_LINKDOWN_LEN, enable);
    fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR, REG_MII_EXT_EEE_CONTROL2);
    fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA, regval);
}

static void fxgmac_phy_mdix_status(struct fxgmac_pdata* pdata, u32 *mdix_status, u32 *mdix_ctrl)
{
    u32 regval;

    fxgmac_read_ephy_reg(pdata, REG_MII_SPEC_CTRL, &regval);
    regval = FXGMAC_GET_REG_BITS(regval, PHY_MII_SPEC_CTRL_CROSS_MD_POS,
                                 PHY_MII_SPEC_CTRL_CROSS_MD_LEN);
    *mdix_ctrl = regval;

    fxgmac_read_ephy_reg(pdata, REG_MII_SPEC_STATUS, &regval);
    regval = FXGMAC_GET_REG_BITS(regval, PHY_MII_SPEC_MIDX_STATUS_POS,
                                 PHY_MII_SPEC_MIDX_STATUS_LEN);
    *mdix_status = regval;
}

static void fxgmac_set_mdix_ctrl(struct fxgmac_pdata* pdata, u32 mdix_ctrl)
{
    u32 regval;

    fxgmac_read_ephy_reg(pdata, REG_MII_SPEC_CTRL, &regval);
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_SPEC_CTRL_CROSS_MD_POS,
                                 PHY_MII_SPEC_CTRL_CROSS_MD_LEN, mdix_ctrl);
    fxgmac_write_ephy_reg(pdata, REG_MII_SPEC_CTRL, regval);
}
#endif
...
void fxgmac_release_phy(struct fxgmac_pdata* pdata)
{
    ...
    if (NIC_PCI_SUBSYSTEMID_FOR_AISTONE == value) {
...
static void fxgmac_config_powerdown(struct fxgmac_pdata* pdata, unsigned int offloadcount, bool magic_en, bool remote_pattern_en)
{
    ...
    /* set bypass pcie reset  */
    regval = readreg(pdata->pAdapter, pdata->base_mem + SYS_RESET_REG);
    regval = FXGMAC_SET_REG_BITS(regval, SYS_RESET_PRESET_ENABLE_SYS_RST_N_POS, SYS_RESET_PRESET_ENABLE_SYS_RST_N_LEN, 0);
...
static int fxgmac_hw_reset(struct fxgmac_pdata *pdata)
{
    u32     regval;
...
static int fxgmac_hw_exit(struct fxgmac_pdata *pdata)
{
    ...
    fxgmac_hw_reset(pdata);
    usleep_range_ex(pdata->pAdapter, 9000, 10000); //usleep_range_ex(pdata->pAdapter, 10, 15);

    /*set not bypass pcie reset */
    regval = readreg(pdata->pAdapter, pdata->base_mem + SYS_RESET_REG);
    regval = FXGMAC_SET_REG_BITS(regval, SYS_RESET_PRESET_ENABLE_SYS_RST_N_POS, SYS_RESET_PRESET_ENABLE_SYS_RST_N_LEN, 1);
    regval = FXGMAC_SET_REG_BITS(regval, SYS_RESET_POS, SYS_RESET_LEN, 0);
    writereg(pdata->pAdapter, regval, pdata->base_mem + SYS_RESET_REG);
...
static int fxgmac_pcie_init(struct fxgmac_pdata* pdata, bool ltr_en, bool aspm_l1ss_en, bool aspm_l1_en, bool aspm_l0s_en)
{
    ...
    cfg_r32(pdata, REG_PCI_SUB_VENDOR_ID, &subsystemID);
    ...
    if (ltr_en) {
        ...
        if (NIC_PCI_SUBSYSTEMID_FOR_MS_LTR == subsystemID
            || NIC_PCI_SUBSYSTEMID_FOR_NON_MS_LTR == subsystemID)
        {
            regval = FXGMAC_SET_REG_BITS(regval, LTR_CTRL_IDLE_THRE_TIMER_POS, LTR_CTRL_IDLE_THRE_TIMER_LEN, LTR_CTRL_IDLE_THRE_TIMER_VAL_LOW_POWER);
        }
        else
        {
    ...
    cfg_r32(pdata, REG_ACK_LATENCY_RELAY_TIMER, &regval);
...
#ifdef ASIC_MODE
    // close AER
    cfg_r32(pdata, REG_CORRECTABLE_ERROR_MASK_REG, &regval);
    ...
    cfg_r32(pdata, REG_L1SUB_TIMING, &regval);
...
#ifndef COMMENT_UNUSED_CODE_TO_REDUCE_SIZE
static void fxgmac_force_led_state(struct fxgmac_pdata* pdata, u32 state)
{
    u32 regval = 0, force_mode = 0;

    switch (state) {
    case FXGMAC_LED_FORCE_INACTIVE:
        fxgmac_config_led_under_active(pdata);
        return;
    case FXGMAC_LED_FORCE_ACTIVE:
        force_mode = REG_MII_EXT_COMMON_LED_CFG_LED_FORCE_MODE1;
        break;
    case FXGMAC_LED_FORCE_ON:
        force_mode = REG_MII_EXT_COMMON_LED_CFG_LED_FORCE_ON;
        break;
    case FXGMAC_LED_FORCE_OFF:
        force_mode = REG_MII_EXT_COMMON_LED_CFG_LED_FORCE_OFF;
        break;
    default:
        return;
    }

    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_EXT_COMMON_LED_CFG_LED0_FORCE_EN_POS,
                                 REG_MII_EXT_COMMON_LED_CFG_LED0_FORCE_EN_LEN,
                                 REG_MII_EXT_COMMON_LED_CFG_LED_FORCE_EN);
    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_EXT_COMMON_LED_CFG_LED1_FORCE_EN_POS,
                                 REG_MII_EXT_COMMON_LED_CFG_LED1_FORCE_EN_LEN,
                                 REG_MII_EXT_COMMON_LED_CFG_LED_FORCE_EN);
    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_EXT_COMMON_LED_CFG_LED2_FORCE_EN_POS,
                                 REG_MII_EXT_COMMON_LED_CFG_LED2_FORCE_EN_LEN,
                                 REG_MII_EXT_COMMON_LED_CFG_LED_FORCE_EN);
    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_EXT_COMMON_LED_CFG_LED0_FORCE_MODE_POS,
                                 REG_MII_EXT_COMMON_LED_CFG_LED0_FORCE_MODE_LEN, force_mode);
    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_EXT_COMMON_LED_CFG_LED1_FORCE_MODE_POS,
                                 REG_MII_EXT_COMMON_LED_CFG_LED1_FORCE_MODE_LEN, force_mode);
    regval = FXGMAC_SET_REG_BITS(regval, REG_MII_EXT_COMMON_LED_CFG_LED2_FORCE_MODE_POS,
                                 REG_MII_EXT_COMMON_LED_CFG_LED2_FORCE_MODE_LEN, force_mode);
    fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR, REG_MII_EXT_COMMON_LED_CFG);
    fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA, regval);
}
#endif
...
void fxgmac_init_hw_ops(struct fxgmac_hw_ops *hw_ops)
{
    ...
    hw_ops->force_led_state = fxgmac_force_led_state;
    hw_ops->phy_eee_feature = fxgmac_phy_eee_feature;
    hw_ops->phy_downgrade_feature = fxgmac_phy_downgrade_feature;
    hw_ops->phy_downgrade_retry_limit = fxgmac_phy_downgrade_retry_Limit;
    hw_ops->phy_fast_linkdown_feature = fxgmac_phy_fast_linkdown_feature;
    hw_ops->phy_mdix_status = fxgmac_phy_mdix_status;
    hw_ops->phy_set_mdix_ctrl = fxgmac_set_mdix_ctrl;
#endif
