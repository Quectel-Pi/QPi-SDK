...
static const struct fxgmac_stats_desc fxgmac_gstring_stats[] = {
    ...
    FXGMAC_STAT("tx_errors", txerrors),
    ...
    FXGMAC_STAT("rx_errors", rxerrors),
				    ...
				    struct ethtool_link_ksettings *cmd)
{
    struct fxgmac_pdata *pdata = netdev_priv(netdev);
    struct fxgmac_hw_ops *hw_ops = &pdata->hw_ops;
    u32 regval, mdix_status, mdix_ctrl;
    int ret;
    ...
    cmd->base.autoneg = pdata->phy_autoeng;
    cmd->base.port = PORT_TP;

    hw_ops->phy_mdix_status(pdata, &mdix_status, &mdix_ctrl);
    cmd->base.eth_tp_mdix = mdix_status ? ETH_TP_MDI_X : ETH_TP_MDI;
    if (mdix_ctrl == 0)
    ...
    if (pdata->phy_autoeng) {
        ...
        if (FXGMAC_GET_REG_BITS(regval, PHY_MII_ADVERTISE_10HALF_POS, PHY_MII_ADVERTISE_10HALF_LEN))
            ethtool_link_ksettings_add_link_mode(cmd, advertising, 10baseT_Half);
        if (FXGMAC_GET_REG_BITS(regval, PHY_MII_ADVERTISE_10FULL_POS, PHY_MII_ADVERTISE_10FULL_LEN))
            ethtool_link_ksettings_add_link_mode(cmd, advertising, 10baseT_Full);
        if (FXGMAC_GET_REG_BITS(regval, PHY_MII_ADVERTISE_100HALF_POS, PHY_MII_ADVERTISE_100HALF_LEN))
            ethtool_link_ksettings_add_link_mode(cmd, advertising, 100baseT_Half);
        if (FXGMAC_GET_REG_BITS(regval, PHY_MII_ADVERTISE_100FULL_POS, PHY_MII_ADVERTISE_100FULL_LEN))
            ethtool_link_ksettings_add_link_mode(cmd, advertising, 100baseT_Full);

        ret = hw_ops->read_ephy_reg(pdata, REG_MII_CTRL1000, &regval);
        if (ret < 0)
                return ret;
        if (FXGMAC_GET_REG_BITS(regval, PHY_MII_CTRL1000_1000FULL_POS, PHY_MII_CTRL1000_1000FULL_LEN))
    ...
    if (pdata->expansion.phy_link) {
        cmd->base.duplex = pdata->phy_duplex;
				    ...
				    const struct ethtool_link_ksettings *cmd)
{
    ...
    struct fxgmac_pdata *pdata = netdev_priv(netdev);
    struct fxgmac_hw_ops *hw_ops = &pdata->hw_ops;
    int ret, mdix = PHY_MII_SPEC_CTRL_CROSS_RESV;
    ...
    if (pdata->phy_speed != SPEED_1000 && pdata->phy_speed != SPEED_100 &&
    ...
    if (pdata->phy_autoeng) {
        ...
        } else if (advertising & ADVERTISED_100baseT_Half) {
            pdata->phy_duplex = DUPLEX_HALF;
            pdata->phy_speed = SPEED_100;
        } else if (advertising & ADVERTISED_10baseT_Full) {
            pdata->phy_duplex = DUPLEX_FULL;
            pdata->phy_speed = SPEED_10;
        } else if (advertising & ADVERTISED_10baseT_Half) {
            pdata->phy_duplex = DUPLEX_HALF;
            pdata->phy_speed = SPEED_10;
        }
    }
    ...
    switch (cmd->base.eth_tp_mdix_ctrl) {
        case ETH_TP_MDI:
            mdix = PHY_MII_SPEC_CTRL_CROSS_MDI;
            break;
        case ETH_TP_MDI_X:
            mdix = PHY_MII_SPEC_CTRL_CROSS_MDIX;
            break;
        case ETH_TP_MDI_AUTO:
            mdix = PHY_MII_SPEC_CTRL_CROSS_AUTO;
            break;
        default:
            break;
    }
    hw_ops->phy_set_mdix_ctrl(pdata, mdix);
...
static int fxgmac_set_phys_id(struct net_device *netdev,
			   enum ethtool_phys_id_state state)
{
    ...
    case ETHTOOL_ID_ACTIVE:
        hw_ops->force_led_state(pdata, FXGMAC_LED_FORCE_ACTIVE);
        return 2;
    case ETHTOOL_ID_ON:
        hw_ops->force_led_state(pdata, FXGMAC_LED_FORCE_ON);
        break;
    case ETHTOOL_ID_OFF:
        hw_ops->force_led_state(pdata, FXGMAC_LED_FORCE_OFF);
        break;
    case ETHTOOL_ID_INACTIVE:
        hw_ops->force_led_state(pdata, FXGMAC_LED_FORCE_INACTIVE);
        break;
    }
...
static int fxgmac_get_phy_tunable(struct net_device *netdev,
                                  const struct ethtool_tunable *tuna, void *data)
{
    ...
    case ETHTOOL_PHY_DOWNSHIFT:
        *val = pdata->phy_downgrade;
        break;
    case ETHTOOL_PHY_FAST_LINK_DOWN:
        *val = pdata->phy_fast_linkdown;
        break;
    default:
        return -EOPNOTSUPP;
    }
...
static int fxgmac_set_phy_tunable(struct net_device *netdev,
                                   const struct ethtool_tunable *tuna, const void *data)
{
    ...
    switch (tuna->id) {
    case ETHTOOL_PHY_DOWNSHIFT:
        /* val is retry limit, down grade is enabled when val is not 0 */
        hw_ops->phy_downgrade_retry_limit(pdata, val);
        pdata->phy_downgrade = val ? 1 : 0;
        hw_ops->phy_downgrade_feature(pdata, pdata->phy_downgrade);
        break;
    case ETHTOOL_PHY_FAST_LINK_DOWN:
        /* 0xff means fast link down off, other values mean on */
        pdata->phy_fast_linkdown = (val == 0xFF) ? 0 : 1;
        hw_ops->phy_fast_linkdown_feature(pdata, pdata->phy_fast_linkdown);
        break;
    case ETHTOOL_PHY_EDPD:
        if (val)
            hw_ops->enable_phy_sleep(pdata);
        else
            hw_ops->disable_phy_sleep(pdata);
        break;
    default:
        return -EOPNOTSUPP;
    }
...
#endif
    .set_phys_id = fxgmac_set_phys_id,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
    .get_phy_tunable = fxgmac_get_phy_tunable,
    .set_phy_tunable = fxgmac_set_phy_tunable,
#endif
};
