...
static int __fxgmac_check_exit(struct net_device *netdev)
{
	struct fxgmac_hw_ops *hw_ops = NULL;
	struct fxgmac_pdata *pdata = NULL;
	...
	if (!parent || !dev_is_pci(parent)) {
	    ...
	    pr_err("Error: Network device not attached to PCI bus\n");
	    return -ENODEV;
	}
	...
	ssid       = pdev->subsystem_device;
	//pr_info("PCIe ID: %04x:%04x (SVID:SSD = %04x:%04x)\n",vendor_id, device_id, svid, ssid);
	
	if (vendor_id != 0x1f0a  || device_id != PCI_DEVICE_ID_FUXI)
	{
		pr_emerg("Error: not yt dev, skip \n");
		//pr_info("PCIe ID: %04x:%04x (SVID:SSD = %04x:%04x)\n",vendor_id, device_id, svid, ssid);
		return -EINVAL;
	}
...
static int fxgmac_panic_notifier(struct notifier_block *nb, 
                            unsigned long event, 
                            void *data) {
   // char *panic_msg = (char *)data;
   // pr_emerg(" CUSTOM PANIC HANDLER TRIGGERED \n");
   // pr_emerg("Panic reason: %s\n", panic_msg);
...
static struct notifier_block panic_nb = {
    .notifier_call = fxgmac_panic_notifier,
    .priority      = INT_MAX
};

static bool is_notifier_registered(struct atomic_notifier_head *nh, 
                                   struct notifier_block *nb) {
    struct notifier_block *iter;

    rcu_read_lock();
    iter = rcu_dereference_raw(nh->head);
    while (iter) {
        if (iter == nb) {
            rcu_read_unlock();
            return true;
        }
        iter = rcu_dereference_raw(iter->next);
    }
    rcu_read_unlock();
...
static int panic_notifier_init(void) {
    ...
    if (is_notifier_registered(&panic_notifier_list, &panic_nb)) {
        pr_info("Panic notifier already registered, skipping\n");
        return 0;
    }
...
#ifdef FXGMAC_DOWNGRADE_DISABLED
    pdata->phy_downgrade = 0;
#else
    pdata->phy_downgrade = 1;
#endif
...
static int fxgmac_pcie_check(struct fxgmac_pdata *pdata)
{
	u32 cfg_pci_cmd, pci_cmd_ok = 0;
	u32 cfg_mem_base = 0;
	u32 cfg_mem_base_hi = 0;
	int ret = 0;

	pci_cmd_ok = FXGMAC_SET_REG_BITS(pci_cmd_ok, PCI_COMAMND_MEM_SPACE_POS,
		PCI_COMAMND_MEM_SPACE_LEN, 1);
	pci_cmd_ok = FXGMAC_SET_REG_BITS(pci_cmd_ok, PCI_COMMAND_MASTER_POS,
		PCI_COMMAND_MASTER_LEN, 1);

	cfg_r32(pdata, REG_PCI_COMMAND, &cfg_pci_cmd);
	if ((cfg_pci_cmd & pci_cmd_ok) != pci_cmd_ok) {
		dev_err(pdata->dev, "PCIe cfg cmd err:%x\n", cfg_pci_cmd);
		ret = -EIO;
	}

	cfg_r32(pdata, REG_MEM_BASE, &cfg_mem_base);
	cfg_r32(pdata, REG_MEM_BASE_HI, &cfg_mem_base_hi);
	if (cfg_mem_base_hi == 0 && ((cfg_mem_base >> 4) == 0)) {
		dev_err(pdata->dev, "PCIe cfg bar err:%x %x\n", cfg_mem_base_hi, cfg_mem_base);
		ret = -EIO;
	}
...
int fxgmac_drv_probe(struct device *dev, struct fxgmac_resources *res)
{
    ...
    ret = fxgmac_pcie_check(pdata);
    if (ret < 0) {
        dev_err(dev, "fxgmac_pcie_check failed.\n");
