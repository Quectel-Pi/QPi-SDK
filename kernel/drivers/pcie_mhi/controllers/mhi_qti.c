...
error_init_pci_arch:
	...
	/*
	 * The controller was registered by mhi_register_controller() above;
	 * unregister it on every error path so a later probe (e.g. PCIe
	 * re-bind/rescan after the modem becomes ready) does not re-use a
	 * stale controller whose mhi_dev->pci_dev points to a freed pci_dev.
	 * Without this, pci_assign_resource() fails with "bogus alignment"
	 * and the modem can never be brought up.
	 */
