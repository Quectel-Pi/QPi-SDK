...
struct msm_dp_display_private {
	...
	/* 2026-08-29 quecpi: next_bridge (USB QMP PHY 桥) 异步轮询等待。
	 * USB PHY 驱动可能晚于 DP probe, 同步 devm_drm_of_get_bridge 会
	 * 返回 -EPROBE_DEFER 阻塞整个 msm_drm bind (亮屏晚 ~1 分钟)。
	 * 改为: probe 不阻塞, 用 delayed_work 轮询等待 bridge, 拿到后
	 * 再设置 next_bridge (DP 运行时启用)。
	 * 快速轮询 (100ms x 100 = 10s) 失败后进入慢速重试 (60s/次),
	 * 避免 USB PHY 极晚加载时 DP 永久失效。 */
	struct delayed_work next_bridge_work;
	bool next_bridge_pending;
	bool next_bridge_slow_retry;
	int poll_count;
};
...
static int msm_dp_hpd_unplug_handle(struct msm_dp_display_private *dp, u32 data)
{
	...
	/*
	 * Clean up hardware immediately (reference driver pattern).
	 * Disable streams and turn off controller before signaling
	 * disconnect, so the DRM disable/unprepare path won't need
	 * to access powered-off registers.
	 */
	...
	/*
	 * Do NOT call pm_runtime_put_sync here. The DP controller must stay
	 * powered so the DRM unprepare path can safely run.
	 * pm_runtime_put_sync in unprepare will power it down.
	 */
	mutex_unlock(&dp->event_mutex);
	return 0;
}
...
void msm_dp_snapshot(struct msm_disp_state *disp_state, struct msm_dp *dp)
{
	...
	if (!msm_dp_display->active_stream_cnt || !dp->link_ready) {
...
static void msm_dp_next_bridge_poll_work(struct work_struct *work)
{
	struct msm_dp_display_private *dp =
		container_of(to_delayed_work(work),
			     struct msm_dp_display_private, next_bridge_work);
	struct msm_dp *msm_dp_display = &dp->msm_dp_display;
	struct device *dev = &msm_dp_display->pdev->dev;
	struct device_node *remote;
	struct drm_bridge *bridge;

	/* 轮询等待外部桥 (USB QMP PHY 的 DP 桥) 注册 */
	remote = of_graph_get_remote_node(dev->of_node, 1, 0);
	bridge = remote ? of_drm_find_bridge(remote) : NULL;
	of_node_put(remote);

	if (bridge) {
		msm_dp_display->next_bridge = bridge;
		dp->next_bridge_pending = false;
		DRM_DEV_INFO(dev, "DP next_bridge ready (polled)\n");
		return;
	}

	/* 快速轮询: 100ms x 100 = 10s */
	if (++dp->poll_count < 100) {
		schedule_delayed_work(&dp->next_bridge_work,
				      msecs_to_jiffies(100));
		return;
	}

	/* 慢速重试: 60s/次, 直到 remove 取消。USB PHY 极晚加载时
	 * 仍能恢复 DP, 且不刷屏 (仅首次超时告警一次)。 */
	if (!dp->next_bridge_slow_retry) {
		dp->next_bridge_slow_retry = true;
		dev_warn(dev, "DP next_bridge not ready after 10s, slow retry every 60s\n");
	}
	schedule_delayed_work(&dp->next_bridge_work,
...
static int msm_dp_display_probe_tail(struct device *dev)
{
	 ...
	 *
	 * 2026-08-29 quecpi: USB QMP PHY 驱动可能晚于 DP probe, 同步
	 * devm_drm_of_get_bridge 会返回 -EPROBE_DEFER, 阻塞整个 msm_drm
	 * component bind (DPU/DSI 主显示被连带拖到 bridge 就绪, 亮屏晚
	 * ~1 分钟)。改为: probe 不阻塞, 先 component_add 让主显示 bind,
	 * 外部桥用 delayed_work 异步轮询等待。
	 */
	...
	if (IS_ERR(dp->next_bridge)) {
		...
		if (dp->is_edp || ret != -ENODEV) {
			if (ret == -EPROBE_DEFER && !dp->is_edp) {
				/* 异步轮询, 不阻塞 probe */
				dp_priv->poll_count = 0;
				dp_priv->next_bridge_slow_retry = false;
				dp_priv->next_bridge_pending = true;
				INIT_DELAYED_WORK(&dp_priv->next_bridge_work,
						  msm_dp_next_bridge_poll_work);
				schedule_delayed_work(&dp_priv->next_bridge_work,
						      msecs_to_jiffies(100));
			} else {
				return ret;
			}
...
static void msm_dp_display_remove(struct platform_device *pdev)
{
	...
	/* 2026-08-29 quecpi: 取消 next_bridge 轮询 work */
	if (dp->next_bridge_pending)
		cancel_delayed_work_sync(&dp->next_bridge_work);
...
void msm_dp_display_disable_helper(struct msm_dp *dp, struct msm_dp_panel *msm_dp_panel)
{
	...
	if (!dp->link_ready)
		pr_info("qdp_fix: skip push_idle link_ready=0\n");
	else
...
void msm_dp_display_atomic_disable(struct msm_dp *msm_dp)
{
	...
	if (!msm_dp->link_ready)
		pr_info("qdp_fix: skip atomic_disable push_idle link_ready=0\n");
	else
...
void msm_dp_display_unprepare(struct msm_dp *msm_dp)
{
	...
	if (!msm_dp->link_ready) {
		/*
		 * Hardware already cleaned up in unplug handler.
		 * Skip register access, just power down and reset state.
		 */
		pr_info("qdp_fix: skip unprepare hw ops link_ready=0\n");
		goto done;
	}
