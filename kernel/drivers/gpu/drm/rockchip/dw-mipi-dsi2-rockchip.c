...
static int dw_mipi_dsi2_encoder_loader_protect(struct rockchip_drm_sub_dev *sub_dev, bool on)
{
	...
	if (dsi2->panel) {
		status = panel_simple_detect(dsi2->panel);
		if (status == connector_status_disconnected) {
			DRM_DEV_INFO_ONCE(dsi2->dev,
					  "DSI panel reports disconnected, skip loader protect\n");
			return 0;
		}
...
static int dw_mipi_dsi2_connector_get_modes(struct drm_connector *connector)
{
	...
	if (dsi2->panel) {
		...
		if (status == connector_status_disconnected) {
			DRM_DEV_INFO_ONCE(dsi2->dev,
					  "DSI panel reports disconnected, skip mode probing\n");
			return 0;
		}
...
dw_mipi_dsi2_connector_detect(struct drm_connector *connector, bool force)
{
	...
	if (dsi2->panel) {
		...
		if (status != connector_status_unknown) {
			DRM_DEV_DEBUG_KMS(dsi2->dev,
					  "DSI panel detect status: %s\n",
			    ...
			    void *data)
{
	...
	ret  = of_find_panel_or_bridge(dev->of_node, &dsi2->panel, &dsi2->bridge);
	if (ret) {
