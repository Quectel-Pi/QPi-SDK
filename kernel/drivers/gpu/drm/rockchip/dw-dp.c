...
struct dw_dp {
	...
	int last_hpd_gpio_level;
	bool last_hpd_gpio_level_valid;
	bool force_hpd;
	bool usbdp_hpd;
...
static bool dw_dp_detect_no_power(struct dw_dp *dp)
{
	...
	if (dp->hpd_gpio) {
		ret = gpiod_get_value_cansleep(dp->hpd_gpio);
		if (ret < 0) {
			dev_err(dp->dev, "failed to read hpd GPIO: %d\n", ret);
			return false;
		}

		if (!dp->last_hpd_gpio_level_valid || dp->last_hpd_gpio_level != ret) {
			dev_info(dp->dev, "HPD gpio level: %s\n", ret ? "high" : "low");
			dp->last_hpd_gpio_level = ret;
			dp->last_hpd_gpio_level_valid = true;
		}
