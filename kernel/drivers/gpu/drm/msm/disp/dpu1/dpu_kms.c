				    ...
				    struct dpu_kms *dpu_kms)
{
	...
	/* 2026-08-29 quecpi: DP 是可选接口 (HDMI 走 DSI->LT9611, DP-1/DP-2
	 * 依赖 USB QMP PHY 桥)。DP 的 next_bridge (USB QMP PHY 桥) 若未
	 * 就绪, _dpu_kms_initialize_displayport 返回 -EPROBE_DEFER, 旧代码
	 * 直接 return 导致整个 msm_drm hw_init 失败重试 -> DPU/DSI 主显示
	 * 被连带阻塞到 next_bridge 就绪 (~51s) -> 亮屏晚 ~1 分钟。
	 * 改为: DP 失败仅告警, 不阻塞 DSI 主显示初始化。 */
	rc = _dpu_kms_initialize_displayport(dev, priv, dpu_kms);
	if (rc) {
		DPU_ERROR("initialize_DP failed, rc = %d (non-fatal, DP optional)\n", rc);
		rc = 0;
	}
