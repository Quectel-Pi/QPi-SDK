...
static void a6xx_gmu_force_off(struct a6xx_gmu *gmu)
{
	...
	/*
	 * NB: we deliberately do NOT disable_irq(gpu->irq) here.  It is
	 * tempting to mask the GPU's own RBBM interrupt during the power
	 * collapse below, but disable_irq() bumps a refcount that
	 * msm_gpu_hw_init() cannot balance (its own disable/enable pair is
	 * self-cancelling), leaving gpu->irq permanently masked after
	 * recovery -> no fence signalling -> perpetual hang/recover loop
	 * -> high-frequency GX power-domain collapse -> LDO17 OCP.
	 * The stale-register-read during collapse is instead guarded in
	 * a6xx_irq() via the gmu->hung flag, which stays set for the whole
	 * recovery window (until a6xx_gmu_resume() clears it).
	 */
