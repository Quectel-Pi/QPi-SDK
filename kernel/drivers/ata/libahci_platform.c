...
int ahci_platform_enable_phys(struct ahci_host_priv *hpriv)
{
	...
	for (i = 0; i < hpriv->nports; i++) {
		/*
		 * Select the protocol before initializing the PHY.  Some PHY
		 * providers program their mode-specific registers from ->init(),
		 * so doing this in the opposite order can initialize a shared PHY
		 * with the previous consumer's mode.
		 */
